Ext.define('view.Z5', {
	...
	initComponent : function() {
	  ...
	  var removeBtn = Ext.widget('mebutton', {
			text : '삭제',
			handler : removeDept,
			code : 'removeBtn',
			hidden : true
		...
		var detailPanel = Ext.create('Ext.tab.Panel', {
					...
					items : [{
							...
							tbar : [{
									xtype : 'tbfill'
									}, createBtn, editBtn, saveBtn,removeBtn],
		...
		function visibleBtn() {
			...
			for (var i = 0; i < btns.length; i++) {
				...
				} else if (visibleBtnCode == removeBtn.code) {
					Common.visibleBtn(removeBtn);
				}
			}
		}
		function create() {
		...
		function clear(){
			...
			Common.empty(code);
			Common.empty(name);
			cityType.setValue(null);
			handleType.setValue(null);
			mngType.setValue(null)
		}
		...
		function removeDept() {
			...
			Ext.MessageBox.confirm('경고', '진짜로 삭제할꺼냐', function(id) {
						if (id == 'yes') {
						console.log(id);
							var obj = {};
							obj.deptCode = code.getValue();
							var postData = new PostData("removeDept", Ext.JSON
											.encode(obj));
							console.log(Ext.JSON.encode(postData));
							AppContext.command(postData, function(reps) {
										Common.alert(reps, clear(), init());
