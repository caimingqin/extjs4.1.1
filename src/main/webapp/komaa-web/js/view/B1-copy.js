Ext.define('view.B1', {
	...
	initComponent : function() {
		...
		me.layout = 'card';
		me.height = 650;// height decide this if layout is card 600 to 650
		// 2013-02-02
		...
		var aa = Ext.widget('fieldset', {
					...
					margin : '0 0 0 5',
					border : false,
					items : [apersonNameAndapersonJumin, insurer,
		...
		var panel = Ext.create('Ext.panel.Panel', {
					...
					tbar : [{
								xtype : 'tbfill'
							}, editBtn, saveBtn, backBtn, addRowBtn],
		...
		var grid = Ext.create('Ext.grid.Panel', {
					...
					tbar : [date1, date2, manageType, customerCdn,
							requestStatusCdn, initBtn, createtBtn],
					store : store,
					columns : [{
								text : '보고서 번호 ',
								flex : 1,
		...
		grid.on('itemdblclick', function(view, record, item, index, e, eOpts) {// 事件监听
					...
					// Common.visible(saveBtn, false);
					initReadOnly(true);
					// Common.visible(editBtn, true);
