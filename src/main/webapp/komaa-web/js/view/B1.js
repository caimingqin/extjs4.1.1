Ext.define('view.B1', {
	...
	initComponent : function() {
		...
		ajaxProxy.url = AppContext.HOST + '/komaa/disLikeQuery';
		ajaxProxy.reader = {
			type : 'json'
		};
		...
		var diseaseTypeComBo1 = Ext.widget('combo', {
					store : ds,
					displayField : 'kuName',
					minChars : 3,
					valueField : 'code',
		...
		var diseaseTypeComBo1Text = Ext.widget('metext', {
					width : 400,
					readOnly:true
				})

		var diseaseTypeComBo2 = Ext.widget('combo', {
					store : ds,
					displayField : 'kuName',
					minChars : 3,
					valueField : 'code',
		...
		var diseaseTypeComBo2Text = Ext.widget('metext', {
					width : 400,
					readOnly:true
				})

		var diseaseTypeComBo3 = Ext.widget('combo', {
					store : ds,
					displayField : 'kuName',
					minChars : 3,
					valueField : 'code',
		...
		var diseaseTypeComBo3Text = Ext.widget('metext', {
					width : 400,
					readOnly:true
				})
     	
        diseaseTypeComBo1.on('select',function(combo, records){
            var name=records[0].get('kuName');
            var code=records[0].get('code');
        	Common.setValue(diseaseTypeComBo1Text,name);
        	diseaseTypeComBo1.setRawValue(code);
        	
        })
        
           diseaseTypeComBo2.on('select',function(combo, records){
            var name=records[0].get('kuName');
            var code=records[0].get('code');
        	Common.setValue(diseaseTypeComBo2Text,name);
        	diseaseTypeComBo2.setRawValue(code);
        	
        })
        
            diseaseTypeComBo3.on('select',function(combo, records){
            var name=records[0].get('kuName');
            var code=records[0].get('code');
        	Common.setValue(diseaseTypeComBo3Text,name);
        	diseaseTypeComBo3.setRawValue(code);
		...
		var aa81 = Ext.widget('fieldset', {
					...
					margin : 0,
					border : false,
					items : [diseaseTypeComBo1,diseaseTypeComBo1Text]
				});
		var aa82 = Ext.widget('fieldset', {
					...
					padding : 0,
					border : false,
					items : [diseaseTypeComBo2, diseaseTypeComBo2Text]
				});
		var aa83 = Ext.widget('fieldset', {
					...
					margin : 0,
					border : false,
					items : [diseaseTypeComBo3, diseaseTypeComBo3Text]
				});
	...
	var aa8 = Ext.widget('mefc', {
		...
		var uploadForm = Ext.create('Ext.form.Panel', {
			...
			items : [uploadFile, {
				...
				text : 'Upload',
				handler : function() {
					var CommandTranslate = uploadForm
							.getComponent('CommandTranslate');
					var datas = {};
					datas.code = "";
		...
		var grid = Ext.create('Ext.grid.Panel', {
			...
			tbar : [date1, date2, manageType, customerCdn, requestStatusCdn,
					likeComboCnd, likeCnd, initBtn, createtBtn],
			store : store,
			columns : [{
						text : '보고서 번호 ',
						flex : 1,
...
//			var dt = new Date();
			...
			if (Common.isNull(date1) && Common.isNull(date2)) {
				...
				} else if (Common.isNull(date1)) {
					date1Time = dt.getTime();
		...
		grid.on('itemdblclick', function(view, record, item, index, e, eOpts) {// 事件监听
					...
					var diseaseTypes = obj.diseaseTypes;
					if (diseaseTypes) {
						if (diseaseTypes.length == 1) {
							Common.setValue(diseaseTypeComBo1,diseaseTypes[0].code);
							Common.setValue(diseaseTypeComBo1Text,diseaseTypes[0].kuName);
						}else if(diseaseTypes.length == 2){
						    Common.setValue(diseaseTypeComBo1,diseaseTypes[0].code);
							Common.setValue(diseaseTypeComBo1Text,diseaseTypes[0].kuName);
							Common.setValue(diseaseTypeComBo2,diseaseTypes[1].code);
							Common.setValue(diseaseTypeComBo2Text,diseaseTypes[1].kuName);
						}else{
							Common.setValue(diseaseTypeComBo1,diseaseTypes[0].code);
							Common.setValue(diseaseTypeComBo1Text,diseaseTypes[0].kuName);
							Common.setValue(diseaseTypeComBo2,diseaseTypes[1].code);
							Common.setValue(diseaseTypeComBo2Text,diseaseTypes[1].kuName);
							Common.setValue(diseaseTypeComBo3,diseaseTypes[2].code);
							Common.setValue(diseaseTypeComBo3Text,diseaseTypes[2].kuName);
						}
					}
					if (obj.insuranceCategory) {
					...
					if (obj.accident) {
						...
						console.log(obj.accident.theDate);
						...
						console.log(obj.accident.person.level.code);
		...
		function clearData() {
			...
			Common.empty(diseaseTypeComBo1);
			Common.empty(diseaseTypeComBo1Text);
			Common.empty(diseaseTypeComBo2);
			Common.empty(diseaseTypeComBo2Text);
			Common.empty(diseaseTypeComBo3);
			Common.empty(diseaseTypeComBo3Text);
			array = [];
		...
		function save() {
			...
			console.log(Common.value(accidentDate) + ","
					+ accidentDate.getValue() + ","
					+ accidentDate.getRawValue());
			...
			obj.apersonsJumin = Common.value(sJumin);
			...
			console.log(Common.value(level));
			...
			var disTypes=[];
			if(Common.value(diseaseTypeComBo1)){
			disTypes.push({"code":Common.value(diseaseTypeComBo1)});
			}
			if(Common.value(diseaseTypeComBo2)){
			disTypes.push({"code":Common.value(diseaseTypeComBo2)});
			}
			if(Common.value(diseaseTypeComBo2)){
			disTypes.push({"code":Common.value(diseaseTypeComBo3)});
			}
			obj.contracts = contracts;
			obj.deseasTypes=disTypes;
