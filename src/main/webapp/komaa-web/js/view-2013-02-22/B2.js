Ext.define('view.B2', {
	...
	var likeCnd = Ext.widget('metext', {
		...
		var grid = Ext.create('Ext.grid.Panel', {
			// region : 'north',
			height : 650,
			tbar : [date1, date2, manageType, customerCdn, requestStatusCdn,likeCnd,likeComboCnd,
					btn],
			store : store,
			columns : [{
						text : '보고서 번호 ',
		...
		// var query;
		// if (teamLeader) {
		// query = "{department.code:"
		// + Common.read(loginUser.department.code) + "}";
		// } else {
		// query = "{user.code:" + Common.read(loginUser.code)
		// + "}";
		// }
		//
		// // console.log(query);
		// // console.log(Common.value(acceptDateSearch).getTime())
		...
		// var loginUserCode = url.substring(url.indexOf("?") + 1,
		// url.length);
		//
		...
		function isTeamLeader() {
			...
			if (loginUser.userRole.code == "03") {
				teamLeader = true;
				// console.log('loginUser.userRole.code = "03"');
				Common.visible(userCombo, true);
				Common.visible(bindSalesManBtn, true);
				Common.visible(acceptSurveyRequstBtn, false);
				Common.visible(rejectSurveyRequstBtn, false);
			} else {
				teamLeader = false;
				Common.visible(userCombo, false);
				Common.visible(bindSalesManBtn, false);
				Common.visible(acceptSurveyRequstBtn, true);
				Common.visible(rejectSurveyRequstBtn, true);
			}
